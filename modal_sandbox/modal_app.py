...
model_image = (
    ...
    #     ]
    # )
    #({"HF_HUB_ENABLE_HF_TRANSFER": "1", "HF_HUB_CACHE": CACHE_DIR})
)
...
@app.cls(
    ...
    secrets=[modal.Secret.from_name("vectara")]
)
class Model:
    # @modal.enter()
    ...
    def index_pdf(self, session_id, target: bytes | list):
        ...
        if isinstance(target, bytes):
            pdf_bytes = target # Keep the original PDF bytes
            images = convert_pdf_to_images.remote(target)
        else:
            images = target
            # If 'target' is a list of images, you'll need to decide how to get the PDF bytes
            # For now, we'll assume 'target' is always bytes when calling this from main()
            pdf_bytes = None # Or handle this case appropriately
        ...
        # TODO: Use Vectara API to process the PDF
        if pdf_bytes:
        ...
        session.pdf_embedded = pdf_embedded
    ...
    def respond_to_message(self, session_id, message):
        ...
        elif session.pdf_embedded is False:
            return "Indexing PDF..."
    ...
    def get_relevant_image(self, message, session, images):
        ...
        # TODO: Use Vectara API to retrieve the most relevant chunks from the message, and display the PDF page
        max_index = 0  # Placeholder for the index of the most relevant image
    ...
    def generate_response(self, message, session, image):
        ...
        # Use a VLM to generate a response based on the message and image
...
pdf_image = (
    ...
    .pip_install("pdf2image==1.17.0", "pillow==11.2.1")
)
